...
async function getData() {
    try {
        ...
        // injecHtml(result.data,"countries-list" , "li")
...
function sortArrayLowHigh(arr, field, direction) {
    if (direction === "low-high") arr.sort((a, b)=>{
        if (a[field] < b[field]) return -1;
        if (a[field] > b[field]) return 1;
        return 0;
    });
    if (direction === "high-low") arr.sort((a, b)=>{
        if (a[field] > b[field]) return -1;
        if (a[field] < b[field]) return 1;
        return 0;
    });
}
function injecHtml(arr, id, htmlSortElement) {
    console.log(id);
    const htmlBaseElement = document.getElementById(id);
    arr.map((item)=>{
        // console.log(item);
        const htmtElement = document.createElement(htmlSortElement);
        ...
        const pRegion = document.createElement('span');
        pRegion.textContent = region;
        const pName = document.createElement('span');
        pName.textContent = name;
        const pPopulation = document.createElement('span');
    ...
    const countryItems = arr.map((item)=>{
        ...
        switch(region){
            case "Africa":
                ...
                break;
            case "Americas":
                if (subregion === "South America") cssClass = "light-green";
                else cssClass = "dark-green";
                break;
            case "Asia":
                cssClass = "red";
                break;
